...
const install = ( name ) => {
    const repository = splitName( name )[1]
    const modspec = join( cd, node_modules, repository + js)
...
const downloadJson = ( name ) => {
    ...
    if ( !fs.folderExists( mods ) ) fs.mkdirSync( mods )
...
const mods = ${ source }
Object.keys( code ).forEach( key => wes.Modules[ key ] = mods[ key ] )
module.exports = require( '${ parent }' )
`
}

modules.exports = install